    ...
    */
    ...
    class TwoDimensionalIntegral {
      public:
        ...
        double operator()(const ext::function<double (double, double)>& f,
                        const std::pair<double, double>& a,
                        const std::pair<double, double>& b) const {
            return (*integratorX_)([&](double x) { return g(f, x, a.second, b.second); },
                                   a.first, b.first);
        }

      private:
        double g(const ext::function<double (double, double)>& f,
               double x, double a, double b) const {
            return (*integratorY_)([&](double y) { return f(x, y); }, a, b);
        }
