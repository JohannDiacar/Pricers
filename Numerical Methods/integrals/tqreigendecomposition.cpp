...
namespace QuantLib {
    ...
    : d_(diag), ev_((calc == WithEigenVector)    ? d_.size() :
        ...
        for (size_t k=n-1; k >=1; --k) {
            while (!offDiagIsZero(k, e)) {
                ...
                for (size_t i=l+1; i <= k && !recoverUnderflow; ++i) {
    ...
    bool TqrEigenDecomposition::offDiagIsZero(size_t k, Array& e) {
