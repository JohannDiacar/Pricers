...
namespace QuantLib {
    ...
    static double rescaleError(double err,
                             const double resultAbs,
                             const double resultAsc) {
        err = std::fabs(err) ;
        if (resultAsc != 0 && err != 0){
            double scale = std::pow((200 * err / resultAsc), 1.5) ;
            if (scale < 1)
                err = resultAsc * scale ;
            else
                err = resultAsc ;
    ...
    static const double w87b[23] = {
     ...
     void GaussKronrodNonAdaptive::setRelativeAccuracy(double relativeAccuracy) { 
        relativeAccuracy_ = relativeAccuracy;
    }
 
 
    double GaussKronrodNonAdaptive::relativeAccuracy() const {
        return relativeAccuracy_;
    }

    GaussKronrodNonAdaptive::GaussKronrodNonAdaptive(double absoluteAccuracy,
    ...
    double
    GaussKronrodNonAdaptive::integrate(const ext::function<double (double)>& f,
                                       double a,
                                       double b) const {
        double result;
        //size_t neval;
    ...
    double
    GaussKronrodAdaptive::integrate(const ext::function<double (double)>& f,
                                    double a,
                                    double b) const {
    ...
    double GaussKronrodAdaptive::integrateRecursively(
    ...
    GaussKronrodAdaptive::GaussKronrodAdaptive(double absoluteAccuracy,
                                               size_t maxEvaluations)
    : Integrator(absoluteAccuracy, maxEvaluations) {
